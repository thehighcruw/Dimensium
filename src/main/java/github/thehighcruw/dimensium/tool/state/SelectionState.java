...
public class SelectionState {
    ...
    public static Set<Long> floodFill(World world, int sx, int sy, int sz, int limit) {
        return floodFill(
            world,
            sx,
            sy,
            ...
            false,
            false,
            MagicSelectToolState.MagicCompareType.BLOCK_STATE,
            MagicSelectToolState.MagicDirection.BOTH);
    }
    ...
    public boolean isTooBigToCopy() {
        int limit = DimensiumConfig.maxCopyVolume;
        return width() > limit || height() > limit || depth() > limit;
    }
    ...
    public void recordOp(String desc) {
        history.add(0, desc);
